...
class APITester:
    ...
    def test_job_status(self, job_id):
        ...
        try:
            response = requests.get(f"{self.base_url}/analysis-status/{job_id}", timeout=10)
    ...
    def test_invalid_endpoints(self):
        ...
        # Test invalid job ID
        try:
            response = requests.get(f"{self.base_url}/analysis-status/invalid-job-id", timeout=10)
